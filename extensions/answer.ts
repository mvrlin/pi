...
function parseExtractionResult(text: string): ExtractionResult | null {
	// Try to find JSON in the response. Some models wrap it in markdown or add prose.
...
/**
 * Fallback extractor for models that do not reliably return JSON.
 */
function extractQuestionsFromText(text: string): ExtractionResult {
	...
	for (const line of text.split("\n")) {
		...
		if (trimmed.includes("?") && /^\s*(?:[-*•]|\d+[.)])?\s*/.test(trimmed)) {
			addQuestion(trimmed);
		}
	}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Use the currently active model for extraction.
			const activeModel = ctx.model;

			// Run extraction with loader UI
			let extractionFailed = false;
			const extractionResult = await ctx.ui.custom<ExtractionResult | null>((tui, theme, _kb, done) => {
				const loader = new BorderedLoader(tui, theme, `Extracting questions using ${activeModel.id}...`);
				...
				const doExtract = async () => {
					...
					if (auth.ok === false) {
						...
						ctx.ui.notify(`Active model auth failed: ${auth.error}`, "error");
						return null;
					}
					...
					const fallbackResult = extractQuestionsFromText(lastAssistantText!);
					if (fallbackResult.questions.length > 0) {
						ctx.ui.notify(
							`Could not parse JSON from ${activeModel.id}; used local question extraction fallback`,
							"info",
						);
					...
					extractionFailed = true;
					ctx.ui.notify(`Could not parse question extraction response from ${activeModel.id}`, "error");
					return null;
				};
				...
				doExtract()
					...
					.catch((error) => {
						...
						ctx.ui.notify(`Question extraction failed with ${activeModel.id}: ${message}`, "error");
						done(null);
					});
